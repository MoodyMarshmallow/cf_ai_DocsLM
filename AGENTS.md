...
Flags:
- `--chat-only` sends a chat request only (no project creation).
- `--index-only` runs indexing only (no chat call).
...
- When linting, ignore `worker-configuration.d.ts` (generated by `wrangler types`).
...
- Workers runtime types for Workflows come from `cloudflare:workers` (run `npx wrangler types` to generate declarations).
